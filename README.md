...
1. **PDA 账户**：使用 Program Derived Address 确保账户安全，防止账户伪造
2. **权限验证**：所有操作都需要用户签名，并验证账户所有权
3. **订阅验证**：API 调用前检查订阅是否有效
4. **溢出保护**：所有数值计算都使用 `checked_*` 方法防止溢出/下溢
5. **重入攻击防护**：遵循 CEI (Checks-Effects-Interactions) 模式，先更新状态再转账
6. **余额检查**：所有转账前都检查余额是否充足
7. **提现限制**：
   - 最小提现金额：0.001 SOL
   - 提现冷却时间：1 小时
8. **操作限额**：
   - 单次最大提交算力：1,000,000 单位
   - 单次最大消耗额度：10,000
9. **验证节点权限**：只有授权的验证节点可以提交算力工作

### 安全审计

详细的安全审计报告请查看 [SECURITY.md](./SECURITY.md)
...
查看完整的部署检查清单：[SECURITY.md](./SECURITY.md#部署前检查)
